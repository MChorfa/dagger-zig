...
pub fn main(init: std.process.Init) !void {
    ...
    const redis_ctr = try client.dag().container();
    const redis_from = try redis_ctr.from("redis:7-alpine");
    const redis = try redis_from.withExposedPort(6379);

    // Convert to service and start
    const redis_svc = try redis.asService();
    _ = try redis_svc.up();
    ...
    const app_ctr_raw = try client.dag().container();
    const app_ctr = try app_ctr_raw.from("alpine:latest");
